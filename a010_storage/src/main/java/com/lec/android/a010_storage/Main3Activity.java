...
public class Main3Activity extends AppCompatActivity {
    ...
    String dbName = "st_file.db"; // 파일의 형태로 DB가 저장
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            //db = helper.getReadableDatabase(); // 읽기전용 DB(ex : SELECT만 사용가능경우)
        }
        catch (SQLException e){
            Log.d("myapp", "데이터베이스를 얻어올 수 없다.");
            finish();
        }
    ...
    void select(){
        ...
        while (c.moveToNext()){
            int id = c.getInt(0); //컬럼인덱스는 0부터 시작
    ...
    void insert(){
        ...
        db.execSQL("INSERT INTO mytable (name) values ('가자미')");
        db.execSQL("INSERT INTO mytable (name) values ('쭈꾸미')");
        db.execSQL("INSERT INTO mytable (name) values ('돌도미')");
        db.execSQL("INSERT INTO mytable (name) values ('탕탕이')");
