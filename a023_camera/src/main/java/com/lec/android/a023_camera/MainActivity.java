...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button.setOnClickListener(new View.OnClickListener() { //촬영
            @Override
    ...
    public void takePicture(){
        ...
        if(intent.resolveActivity(getPackageManager()) != null){// 위 액티비티가 가용한지 여부 체크한뒤
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        //사진 촬영 앱의 사진 결과를 받아오게 되면 수행하는 메소드
        if(requestCode == 101 && resultCode == RESULT_OK){
            //저장된 파일을 -> BitMap 객체로 만들고 -> ImageView에 띄우기
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inSampleSize = 8; //조금 작은 사이즈로 resample 을 위한 samplesize
            Bitmap bitmap = BitmapFactory.decodeFile(file.getAbsolutePath(), options);
            imageView.setImageBitmap(bitmap); //세팅
        }
