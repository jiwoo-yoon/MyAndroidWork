...
public class MainActivity extends AppCompatActivity {
    ...
    class BackThread extends Thread{
        ...
        public void run() {
            while (true){
                ...
                //작업쓰레드에서 메인 UI 직접 건드릴수 없다, 그래서 Handler를 이용!

            }
        }
    }

    //2. Runnable 을 implements
    class BackRunnable implements Runnable{
        @Override
        public void run() {
            while (true){
                backValue2 += 2; // 작업스레드값 증가
    ...
    public void mOnClick2(View v){
        //Thread.currentThread().stop(); <-- 안드로이드는 메인 쓰레드를 강제종료 시킬 수 없다.
    }//end monclick2
