...
public class PhonebookAdapter extends RecyclerView.Adapter<PhonebookAdapter.ViewHolder> {
    ...
    static PhonebookAdapter adapter; // 뷰홀더도 스태틱이너클래스라 만들수있는것, 데이터삭제를 하기 위해서는 폰북어댑터가 관리하고 있는 리스트값들이 필요해서 자기자신을 받는 어댑터생성자를 만들어서 진행

    //Adapter 생성자
    public PhonebookAdapter() {this.adapter = this;}


    //~메인클래스에서 어댑터를 상속시켜주면서 생긴 메소드들
...
//  이로부터 Context 객체를 뽑아내어 Layout inflation 을 해야 한다.
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        //xml layout -> inflator를 통해 -> view layout 변환
        //주어진 ViewGroup 으로부터 LayoutInflator 추출
        LayoutInflater inf = LayoutInflater.from(parent.getContext()); // inflator 추출
        //준비된 레이아웃(xml)으로부터 View를 만들어 ViewGroup에 붙이고
        //그렇게 만들어진 View를 리턴한다.
        View itemView = inf.inflate(R.layout.item, parent, false); //뷰객체로 변환
...
//  리스트 상에 몇번째 데이터인지에 대한 정보 (position) 가 넘어온다.
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Phonebook item = items.get(position);//List<>의 get(), 포지션은 몇번
        holder.setItem(item);
    }
    ...
    static class ViewHolder extends RecyclerView.ViewHolder{
        ...
        //생성자 필수
        public ViewHolder(@NonNull View itemView) { //item.xml에서 만든 내용들이 담긴다.(view객체가전달)
            super(itemView);
            ...
            btnDelItem.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    //position 정보가 필요
                    //adapter로 부터 데이터 삭제도 진행되어야 한다.
                    adapter.removeItem(getAdapterPosition());//데이터 삭제가 진행되면 변경(수정,삭제, 추가)된 데이터내용이 adapter에 반영되어야 정상적으로 동작한다.
                    adapter.notifyDataSetChanged(); // <- 반영해주는 메소드
                }
            });

            //클릭 리스너 장착(아이템자체를 클릭했을때), 상세페이지동작추가
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Intent intent = new Intent(v.getContext(), PhonebookDetail.class); //액티비티클래스가 아니기에 v.으로 네이밍
                    intent.putExtra("pb", adapter.getItem(position));
                    v.getContext().startActivity(intent); //v.으로 네이밍
                }
            });
        }//end 생성자
