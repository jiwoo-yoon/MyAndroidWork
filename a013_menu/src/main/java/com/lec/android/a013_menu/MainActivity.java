...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_main1, menu); //메뉴 리소스(xml)로부터 Menu생성
        Log.d("myapp", "onCreateOptionsMenu - 최초메뉴키를 눌렀을때 호출됨");

        //2> 심지어 MenuItem 을 동적으로 추가 가능하다, add() MenuItem 리턴
        MenuItem item1 = menu.add(1, MENUITEM_YELLOW, 100, "노랑");
        MenuItem item2 = menu.add(2, MENUITEM_ORANGE, 100, "오렌지");
        MenuItem item3 = menu.add(3, MENUITEM_CYAN, 100, "시안");


        return true; //false 리턴시 메뉴는 보이지않음
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        //메뉴가 보여질떄마다 '로그인', '로그아웃'이 enable/disable 토글되어 보이기
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        Log.d("myapp", "onOptionsItemSelected - 메뉴항목을 클릭했을 떄 호출됨");

        showInfo(item);

        //2> 특정 MenuItem 에 대한 동작, 각각의 색상이 눌렸을때
