...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //SoundPool 객체 생성, 롤리팝이상,이하를 구분지어줘야 더 좋음
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
            //롤리팝(API21) 이상에서는 아래와 같이 SoundPool 생성
            sp = new SoundPool.Builder().setMaxStreams(10).build();
        }else{
            sp = new SoundPool(1, // 재생음향최대개수
                    AudioManager.STREAM_MUSIC,  //재생미디어타입
                    0); //재생품질(안씀,디폴트)
        }

        //SoundPool에 음향리소스들을 담는다.(load)
        for (int i = 0; i < SOUND_RES.length; i++){
            soundId[i] = sp.load(this, //현재 화면의 제어권자
                    SOUND_RES[i], //음원파일리소스
                    1); // 재생 우선순위
            //load를 하면 음향리소스id값을 준다, id값을 받는 변수를 만들고 담아준다.
        }
